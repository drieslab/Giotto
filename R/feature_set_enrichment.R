...
#' @param path_to_GSEA path to GSEA command line executable, e.g. gsea-XXX.jar. See details (1.) for more information.
#' @param GSEA_dataset path to a Human/Mouse collection from GSEA, e.g. Hallmarks C1. See details (2.) for more information.
#' @param GSEA_ranked_file path to .rnk file for GSEA. See details (3.) for more information
#' @param output_folder path to which the GSEA results will be saved. Default is current working directory.
#' @param name_analysis_folder default output subdirectory prefix to which results are saved.
#'                             Will live within output_folder; equivalent of "Analysis Name" in GSEA Application.
#' @param collapse only 'false' is supported. This will use your dataset as-is, in the original format.
#' @param mode option selected in Advanced Field "Collapsing Mode for Probe Sets => 1 gene"
#' @param norm normalization mode; only meandiv is supported.
#' @param nperm number of permutations, default 1000
#' @param scoring_scheme Default "weighted", equivalent of "enrichment statistic" in GSEA Application
#' @param plot_top_x Default 20, number of enrichment plots to produce.
#' @param set_max default 500, equivalent to "max size; exclude larger sets" in Basic Fields in GSEA Application
#' @param set_min default 15, equivalent to "min size; exclude smaller sets" in Basic Fields in GSEA Application
#' @details
#' NECESSARY PREREQUISITES
#' 1. download and install the COMMAND line (all platforms) gsea-XXX.jar
#' https://www.gsea-msigdb.org/gsea/downloads.jsp
#' 1.1. download zip file
#' 1.2. unzip and move to known location (e.g. in path/to/your/applications/gsea/GSEA_4.3.2)
#'
#' 2. download the Human and Mouse collections
#' https://www.gsea-msigdb.org/gsea/msigdb/index.jsp or zipped folder https://www.gsea-msigdb.org/gsea/downloads.jsp (all downloaded)
#'
...
doFeatureSetEnrichment <- function(dryrun = TRUE,
  ...
  if(is.null(output_folder)) output_folder = paste0(getwd(), "/Feature_set_enrichment_results/")

  if(!dir.exists(output_folder)) {
    wrap_msg(paste0("Directory does not yet exist. Creating directory at:", output_folder))
  ...
  if(is.null(GSEA_dataset)) {
    warning('Path to a GSEA dataset needs to be provided, only dryrun will work for testing')
    dont_run = TRUE
  ...
  if(my_os == 'windows') {
    execution_path = paste0(path_to_GSEA,'/','gsea-cli.bat', ' ', operation)
  } else {
    execution_path = paste0(path_to_GSEA,'/','gsea-cli.sh', ' ', operation)
  }

  created_command = sprintf("%s \\
            -gmx %s \\
            -collapse %s \\
            -mode %s \\
            -norm %s \\
  ...
  if(isTRUE(dryrun) | isTRUE(dont_run)) {
    wrap_msg('DRYRUN VERSION \n \n')
    print(created_command)
  } else {
    wrap_msg('START GSEA RUN \n \n')
