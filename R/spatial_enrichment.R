...
spatialAutoCorLocal <- function(gobject = NULL,
    ...
    if (isTRUE(return_gobject)) {
        if (isTRUE(verbose)) {
            wrap_msg("Attaching ", method_select, ' results as spatial enrichment: "',
                enrich_name, '"',
                sep = ""
            )
        }
...
.run_spat_autocor_global <- function(use_values,
    ...
    if (test_method == "none") {
        ...
        colnames(res_dt) <- c("feat_ID", cor_name, paste0(cor_name, "_", test_method))
    }
    return(res_dt)
}
...
.evaluate_autocor_input <- function(gobject,
    ...
    if (isTRUE(use_expr)) {
        # EXPR====================================================================#
        values <- match.arg(expression_values, unique(c("normalized", "scaled", "custom", expression_values)))
        use_values <- get_expression_values(
            gobject = gobject,
            spat_unit = spat_unit,
            feat_type = feat_type,
            values = values,
